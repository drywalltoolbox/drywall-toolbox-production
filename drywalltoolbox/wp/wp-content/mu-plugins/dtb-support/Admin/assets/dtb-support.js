  ...
  window.dtbSupport = {
    ...
    bindLegacyReplyForm: function () {
      ...
      $form.on('submit', function (e) {
        ...
        self.api('POST', 'tickets/' + ticketId + '/reply', {
          message: body,
    ...
    switchQueue: function (queue) {
      ...
      $('#dtb-active-queue-label,#dtb-context-queue').text(this.queueLabels[this.activeQueue] || this.activeQueue);
      this.loadTickets(this.activeQueue, this.filters);
    },
    ...
    loadKpis: function () {
      return this.api('GET', 'kpis').done(function (kpis) {
        var items = [
          ['Active', kpis.active_total || kpis.total || 0, 'ok'],
        ...
        $('#dtb-kpi-strip').html(items.map(function (item) {
          return '<div class="dtb-kpi dtb-kpi--' + item[2] + '"><div class="dtb-kpi__val">' + esc(item[1]) + '</div><div class="dtb-kpi__label">' + esc(item[0]) + '</div></div>';
        }).join(''));
      });
    },
    ...
    loadTickets: function (queue, filters) {
      ...
      return this.api('GET', 'tickets', params).done(function (res) {
    ...
    renderTicketRow: function (ticket) {
      ...
      return '<tr id="dtb-row-' + ticket.id + '" class="' + selected + '">' +
        ...
        '<td>' + esc(assigned) + '</td>' +
        '<td>' + this.renderActionBadge(ticket) + '</td>' +
        '<td>' + (lastReply ? esc(this.formatAge(lastReply)) : '-') + '</td>' +
        '<td><div class="dtb-row-actions">' + rowActions + '</div></td>' +
        '</tr>';
    },
    ...
    openTicket: function (ticketId) {
      ...
      this.api('GET', 'tickets/' + ticketId).done(function (res) {
    ...
    renderDetail: function (ticket, events) {
      if (!ticket) return;
      var detail = '<div class="dtb-topbar dtb-topbar--detail"><div class="dtb-topbar__ticket-wrap"><h2 class="dtb-topbar__title">' + esc(ticket.ticket_number) + ' / ' + esc(ticket.subject) + '</h2><div class="dtb-detail-chip-row"><span class="dtb-detail-chip dtb-detail-chip--status">' + esc(ticket.status_label || ticket.status) + '</span><span class="dtb-detail-chip dtb-detail-chip--priority">' + esc(ticket.priority_label || ticket.priority) + '</span><span class="dtb-detail-chip dtb-detail-chip--action">Action: ' + esc(ticket.action_state_label || ticket.action_state || 'On Track') + '</span></div></div><div class="dtb-detail-top-actions"><button class="dtb-btn dtb-btn--ghost dtb-btn--sm" onclick="dtbSupport.copyTicketSummary()">Copy Summary</button><button class="dtb-btn dtb-btn--ghost dtb-btn--sm" onclick="dtbSupport.closeDetail(event)">Close</button></div></div><div class="dtb-detail-shell"><div class="dtb-detail-main">' + this.renderInsightStrip(ticket, events) + '<div id="dtb-thread-panel-host">' + this.renderThreadPanel(events) + '</div>' + this.renderComposer(ticket) + '</div><aside class="dtb-detail-sidebar">' + this.renderContextSidebar(ticket) + '</aside></div>';
    ...
    renderThread: function (events) {
      ...
      if (!filtered.length) return '<div class="dtb-empty"><p class="dtb-empty__msg">No events for this filter.</p><p class="dtb-empty__sub">Try viewing all timeline activity.</p></div>';
      return filtered.map(function (event) { return self.renderEvent(event); }).join('');
    },
    ...
    renderComposer: function (ticket) {
      return '<div class="dtb-composer' + (this.composerMode === 'note' ? ' dtb-composer--internal' : '') + '" style="margin-top:16px"><div class="dtb-composer__mode-tabs"><button class="dtb-composer__mode-tab ' + (this.composerMode === 'reply' ? 'is-active' : '') + '" onclick="dtbSupport.setComposerMode(\'reply\')">Reply</button><button class="dtb-composer__mode-tab ' + (this.composerMode === 'note' ? 'is-active is-active--internal' : '') + '" onclick="dtbSupport.setComposerMode(\'note\')">Internal Note</button></div><div class="dtb-composer__body"><textarea id="dtb-composer-body" class="dtb-composer__textarea" placeholder="' + (this.composerMode === 'note' ? 'Add an internal note...' : 'Write a reply...') + '"></textarea></div><div class="dtb-composer__footer"><span class="dtb-internal-label" ' + (this.composerMode === 'note' ? '' : 'style="display:none"') + '>Internal</span><div class="dtb-macro-host"><button class="dtb-btn dtb-btn--ghost dtb-btn--sm" onclick="dtbSupport.showMacroPicker()">Macros</button><div id="dtb-macro-picker" class="dtb-macro-picker" style="display:none"></div></div><div style="margin-left:auto;display:flex;gap:8px"><button class="dtb-btn dtb-btn--ghost dtb-btn--sm" onclick="dtbSupport.' + (this.composerMode === 'note' ? 'addNote' : 'sendReply') + '(' + ticket.id + ')">Send</button><button class="dtb-btn dtb-btn--primary dtb-btn--sm" onclick="dtbSupport.sendAndResolve(' + ticket.id + ')" ' + (this.composerMode === 'note' ? 'style="display:none"' : '') + '>Send and Resolve</button></div></div></div>';
    },
    ...
    renderStatusOptions: function (current) {
      return [['open', 'Open'], ['pending_staff', 'Pending Staff'], ['pending_customer', 'Pending Customer'], ['in_progress', 'In Progress'], ['resolved', 'Resolved'], ['closed', 'Closed']].map(function (option) {
        return '<option value="' + esc(option[0]) + '" ' + (option[0] === current ? 'selected' : '') + '>' + esc(option[1]) + '</option>';
      }).join('');
    },

    renderPriorityOptions: function (current) {
      return [['low', 'Low'], ['normal', 'Normal'], ['high', 'High'], ['urgent', 'Urgent']].map(function (option) {
        return '<option value="' + esc(option[0]) + '" ' + (option[0] === current ? 'selected' : '') + '>' + esc(option[1]) + '</option>';
      }).join('');
    },
    ...
    applyTicketWorkflow: function (ticketId) {
      ...
      this.updateTicket(ticketId, payload, 'Ticket updated');
    },

    setComposerMode: function (mode) {
      this.composerMode = mode === 'note' ? 'note' : 'reply';
      if (this.activeTicketId) this.openTicket(this.activeTicketId);
    },
    ...
    sendReply: function (ticketId) {
      return this.submitMessage(ticketId, false, 'Reply sent');
    },

    addNote: function (ticketId) {
      return this.submitMessage(ticketId, true, 'Internal note added');
    },

    submitMessage: function (ticketId, isInternal, successMessage) {
      var self = this;
      var body = this.getComposerBody();
      if (!$.trim(body)) {
        this.showToast(isInternal ? 'Note body required' : 'Reply body required', 'info');
        return $.Deferred().reject().promise();
      }
      return this.api('POST', 'tickets/' + ticketId + '/reply', { message: body, is_internal: Boolean(isInternal) }).done(function () {
        self.showToast(successMessage, 'success');
        self.openTicket(ticketId);
        self.loadTickets(self.activeQueue, self.filters);
      }).fail(function (xhr) {
        self.showToast((xhr.responseJSON && xhr.responseJSON.message) || 'Message failed', 'error');
      });
    },

    sendAndResolve: function (ticketId) {
      var self = this;
      this.sendReply(ticketId).done(function () { self.setStatus(ticketId, 'resolved'); });
    },

    loadMacros: function () {
      var self = this;
      return this.api('GET', 'macros').done(function (res) { self.macros = res.macros || []; });
    },
    ...
    showMacroPicker: function () {
      ...
      if (!this.macros.length) { this.loadMacros().done(function () { self.showMacroPicker(); }); return; }
      $picker.html(this.macros.map(function (macro) { return '<div class="dtb-macro-item" onclick="dtbSupport.applyMacro(' + macro.id + ')"><div class="dtb-macro-item__name">' + esc(macro.macro_name) + '</div><div class="dtb-macro-item__cat">' + esc(macro.category) + '</div></div>'; }).join('')).show();
    },

    applyMacro: function (macroId) {
      var macro = (this.macros || []).filter(function (item) { return parseInt(item.id, 10) === parseInt(macroId, 10); })[0];
      if (!macro) return;
      var body = this.renderTemplate(macro.body_template || '');
      $('#dtb-composer-body').val(body);
      $('#dtb-macro-picker').hide();
      this.showToast('Macro inserted', 'success');
    },
    ...
    updateTicket: function (ticketId, payload, successMessage) {
      var self = this;
      return this.api('PATCH', 'tickets/' + ticketId, payload).done(function () {
        self.showToast(successMessage || 'Ticket updated', 'success');
        self.refresh();
      }).fail(function (xhr) {
    ...
    assignToMe: function (ticketId) { return this.updateTicket(ticketId, { assigned_user_id: cfg.currentUserId }, 'Ticket assigned'); },
    assignTicket: function (ticketId, userId) { return userId ? this.updateTicket(ticketId, { assigned_user_id: parseInt(userId, 10) }, 'Ticket assigned') : this.unassignTicket(ticketId); },
    unassignTicket: function (ticketId) { return this.applyBulkActionToIds([ticketId], 'unassign', '', 'Ticket unassigned'); },
    setStatus: function (ticketId, status) { return this.updateTicket(ticketId, { status: status }, 'Status updated'); },
    setPriority: function (ticketId, priority) { return this.updateTicket(ticketId, { priority: priority }, 'Priority updated'); },
    resolveTicket: function (ticketId) { return this.setStatus(ticketId, 'resolved'); },
    transitionTicket: function (ticketId, status, button) { if (button) button.disabled = true; return this.setStatus(ticketId, status); },
    request: function (path, method, data) { return this.api(method || 'GET', String(path || '').replace(/^\/?support\//, ''), data); },
    ...
    snoozeTicket: function (ticketId, until, reason) {
      ...
      return this.api('POST', 'tickets/' + ticketId + '/snooze', { snooze_until: snoozeUntil, reason: snoozeReason }).done(function () { self.showToast('Ticket snoozed', 'success'); self.refresh(); });
    },

    unsnoozeTicket: function (ticketId) {
      var self = this;
      return this.api('DELETE', 'tickets/' + ticketId + '/snooze').done(function () { self.showToast('Ticket unsnoozed', 'success'); self.refresh(); }).fail(function (xhr) { self.showToast((xhr.responseJSON && xhr.responseJSON.message) || 'Unsnooze failed', 'error'); });
    },

    quickSnoozeHours: function (ticketId, hours) {
      this.snoozeTicket(ticketId, toUtcMysql(new Date(Date.now() + hours * 60 * 60 * 1000)), 'Quick snooze: ' + hours + 'h');
    },
    ...
    quickFollowupHours: function (ticketId, hours) {
      var self = this;
      return this.api('POST', 'tickets/' + ticketId + '/followup', { followup_due_at: toUtcMysql(new Date(Date.now() + hours * 60 * 60 * 1000)), note: 'Quick follow-up set for ' + hours + 'h' }).done(function () { self.showToast('Follow-up scheduled', 'success'); self.refresh(); });
    },
    ...
    quickFollowupTomorrow: function (ticketId) {
      ...
      return this.api('POST', 'tickets/' + ticketId + '/followup', { followup_due_at: toUtcMysql(date), note: 'Quick follow-up set for tomorrow 10AM local' }).done(function () { dtbSupport.showToast('Follow-up scheduled', 'success'); dtbSupport.refresh(); });
    },

    toggleBulkSelect: function (ticketId, checked) {
      if (checked) this.selectedTickets[ticketId] = true;
      else delete this.selectedTickets[ticketId];
    ...
    selectAll: function (checked) {
      ...
      if (!checked) this.clearSelection();
    },
    ...
    applyBulkActionToIds: function (ids, action, value, successMessage) {
      var self = this;
      return this.api('POST', 'bulk', { ticket_ids: ids, action: action, value: value || '' }).done(function (res) {
        self.showToast(successMessage || ('Processed ' + ((res.processed || res.updated || []).length) + ' tickets'), 'success');
        self.clearSelection();
        self.refresh();
      }).fail(function (xhr) { self.showToast((xhr.responseJSON && xhr.responseJSON.message) || 'Bulk action failed', 'error'); });
    },

    applyBulkAction: function (action, value) {
      var ids = Object.keys(this.selectedTickets).map(function (id) { return parseInt(id, 10); });
      if (!ids.length) { this.showToast('Select tickets first', 'info'); return; }
      return this.applyBulkActionToIds(ids, action, value);
    },
    ...
    prevPage: function () { if (this.page > 1) { this.page--; this.loadTickets(this.activeQueue, this.filters); } },
    nextPage: function () { if (this.page < this.pageCount) { this.page++; this.loadTickets(this.activeQueue, this.filters); } },

    initKeyboardShortcuts: function () {
      var self = this;
      $(document).on('keydown', function (e) {
        if ($(document.activeElement).is('input,textarea,select,[contenteditable]')) return;
        if (e.key === '/') { e.preventDefault(); $('#dtb-search').focus(); }
        if (e.key === 'r') self.refresh();
        if (e.key === 'Escape') self.closeDetail({ target: { id: 'dtb-detail-overlay' } });
      });
    },

    closeDetail: function (event) {
      if (event && event.target && event.target.id !== 'dtb-detail-overlay' && event.target.id !== 'dtb-detail-panel') return;
    ...
    api: function (method, path, data) {
      ...
      var payload = data;
      if (method === 'GET' && payload) { url += (url.indexOf('?') === -1 ? '?' : '&') + $.param(payload); payload = undefined; }
      return $.ajax({ url: url, method: method, contentType: 'application/json', data: payload ? JSON.stringify(payload) : undefined, beforeSend: function (xhr) { xhr.setRequestHeader('X-WP-Nonce', cfg.nonce || ''); } });
    },
    ...
    copyTicketSummary: function () {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(function () { self.showToast('Ticket summary copied', 'success'); }).catch(function () { self.fallbackCopyText(text); });
        return;
      }
    ...
    fallbackCopyText: function (text) {
      ...
      $textarea[0].select();
      try { document.execCommand('copy'); this.showToast('Ticket summary copied', 'success'); } catch (err) { this.showToast('Copy failed', 'error'); }
      $textarea.remove();
    },
    ...
    buildTicketSummary: function () {
      var ticket = this.currentTicket || {};
      var assigned = ticket.assigned_user && ticket.assigned_user.display_name ? ticket.assigned_user.display_name : 'Unassigned';
      var events = (this.currentDisplayEvents || this.currentEvents || []).slice(-5).map(function (event) { return '- [' + (event.event_label || event.event_type || 'Event') + '] ' + (event.summary || event.body || 'No details'); });
    ...
    formatAge: function (datetime) {
      var ts = Date.parse(String(datetime).replace(' ', 'T') + 'Z');
      if (!ts) return '-';
    ...
    formatDateTime: function (datetime) {
      ...
      if (!parsed) return String(datetime);
      return new Date(parsed).toLocaleString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
    },
    ...
    startAutoRefresh: function () {
      ...
      this.autoRefreshTimer = setInterval(function () { self.loadQueueCounts(); self.loadKpis(); }, interval);
    },
