...
(function () {
	...
	function isMobile() { return Boolean(mobileQuery && mobileQuery.matches); }
	...
	function methodKey(method) {
		...
		return String((method ? method.className : '') + ' ' + (input ? input.id + ' ' + input.value + ' ' + input.name : '') + ' ' + (label ? label.textContent : '')).toLowerCase();
	}
	...
	function orderTotalText() {
		var total = document.querySelector('.dtb-op-card table.shop_table tfoot tr.order-total td') || document.querySelector('.dtb-op-card table.shop_table tfoot tr:last-child td');
	...
	function splitNameAndSku(value) {
		...
		if (skuMatch) { sku = skuMatch[1]; raw = raw.replace(/SKU\s*:?\s*[^\s]+/i, ' '); }
	...
	function buildSummaryCard() {
		...
		var signature = compactText(table).slice(0, 2000);
		...
		Array.prototype.slice.call(table.querySelectorAll('tfoot tr')).forEach(function (row) {
			...
			var final = row.classList.contains('order-total') || /^total$/i.test(label);
			var totalRow = el('div', 'dtb-op-summary-total-row' + (final ? ' dtb-op-summary-total-row--final' : ''));
	...
	function syncMethods() {
		...
		Array.prototype.slice.call(host.querySelectorAll('.wc_payment_method')).forEach(function (method) {
			...
			method.classList.toggle('dtb-op-payment-has-detail', gatewayHasDetails(method));
	...
	function syncPayButton() {
		...
		var label = total ? 'Pay securely — ' + total : 'Pay securely';
		...
		if (!placeOrder.querySelector('.dtb-op-cta-trust')) placeOrder.append(el('div', 'dtb-op-cta-trust', 'Encrypted payment. Gateway confirmation completes the charge.'));
	}

	function setSubmitting(next) {
		var button = document.querySelector('.dtb-op-card #place_order');
		submitting = Boolean(next);
		document.body.classList.toggle('dtb-op-payment-submitting', submitting);
		if (!button) return;
		if (submitting) {
	...
	function enhanceNotices() {
		...
		notices.forEach(function (notice) {
			if (!notice.querySelector('.dtb-op-notice-title')) notice.insertBefore(el('strong', 'dtb-op-notice-title', notice.classList.contains('woocommerce-error') ? 'Payment could not be completed' : 'Checkout update'), notice.firstChild);
		});
	...
	function repairLogoFallback() {
		...
		var wordmark = logo && logo.parentElement ? logo.parentElement.querySelector('.dtb-op-wordmark') : null;
		if (!logo || !wordmark) return;
		var fallback = function () { if (!logo.naturalWidth) { logo.style.display = 'none'; wordmark.style.display = 'inline-flex'; } };
		logo.addEventListener('error', fallback, { once: true });
		window.setTimeout(fallback, 350);
	}
	...
	function sync() {
		...
		Array.prototype.slice.call(document.querySelectorAll('.dtb-payment-sheet-current')).forEach(function (node) { node.classList.remove('dtb-payment-sheet-current'); });
		repairLogoFallback();
		buildSummaryCard();
		syncMethods();
		syncPayButton();
	...
	window.addEventListener('pagehide', function () { if (observer) observer.disconnect(); observer = null; });
}());
