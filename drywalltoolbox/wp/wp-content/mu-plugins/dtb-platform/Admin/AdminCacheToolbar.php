...
function dtb_admin_cache_toolbar_register( WP_Admin_Bar $wp_admin_bar ): void {
	...
	$wp_admin_bar->add_node( [
		...
		'href'  => $clear_url,
		'meta'  => [
			'title' => __( 'Clear all DTB, WordPress, WooCommerce, OPcache, HostGator page cache, and supported CDN cache layers.', 'drywall-toolbox' ),
	...
	$page_cache = DTB_CacheOperationsService::page_cache_status();
	if ( ! empty( $page_cache['settings_url'] ) ) {
		$wp_admin_bar->add_node( [
			'id'     => 'dtb-clear-caches-hostgator',
			'parent' => 'dtb-clear-caches',
			'title'  => __( 'HostGator cache settings', 'drywall-toolbox' ),
			'href'   => esc_url_raw( $page_cache['settings_url'] ),
		] );
	}
}
