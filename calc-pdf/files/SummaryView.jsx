...
import { exportCalculatorReportPdf, buildReportPayload } from '../../api/calculatorExport'

export default function SummaryView({ data, onProjectUpdate }) {
  const { sheets, mud, tape, bead, screws, project = {} } = data
  const projectName = project.jobName
  ...
  const handleProjectField = (field) => (e) => {
    const next = { ...projectDraft, [field]: e.target.value }
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      await exportCalculatorReportPdf(payload)
      setPdfStatus('idle')
    } catch (err) {
      setPdfStatus('error')
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <input
            ...
            value={projectDraft.jobName}
            onChange={handleProjectField('jobName')}
            className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
          <input
            type="text"
            placeholder="Job address"
            value={projectDraft.jobAddress}
            onChange={handleProjectField('jobAddress')}
            className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
          <input
            type="text"
            placeholder="Contractor name"
            value={projectDraft.contractorName}
            onChange={handleProjectField('contractorName')}
            className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
          <input
            type="text"
            placeholder="Estimator name"
            value={projectDraft.estimatorName}
            onChange={handleProjectField('estimatorName')}
            className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
          <textarea
            placeholder="Notes (optional)"
            value={projectDraft.notes}
            onChange={handleProjectField('notes')}
            rows={2}
            className="sm:col-span-2 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 pt-2">
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-accent-500 hover:bg-accent-600 text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {pdfStatus === 'loading' ? (
          ...
          {pdfStatus === 'loading' ? 'Generating…' : 'Export PDF'}
        </button>
        <button
      ...
      {pdfStatus === 'error' && (
        <p className="text-sm text-red-600 text-right">{pdfError}</p>
      )}
