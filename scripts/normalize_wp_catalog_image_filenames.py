...
SUMMARY_PATH = REPORTS_DIR / "wp_catalog_image_filename_normalization_summary.json"
MAP_PATH = REPORTS_DIR / "wp_catalog_image_filename_renames.csv"
...
def load_catalog_skus() -> dict[str, dict[str, str]]:
    ...
    with CATALOG_PATH.open(encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            sku_key = raw_sku.lower()
            if not brand_slug or not raw_sku:
                continue
            sku_case_by_brand.setdefault(brand_slug, {}).setdefault(sku_key, raw_sku)
...
def build_rename_map(
    ...
    for path in sorted(IMAGES_DIR.glob("*.webp")):
        ...
        if not parsed:
            continue
        brand_slug, extracted_sku, seq = parsed
        sku_key = extracted_sku.replace(".", "-").lower()
        canonical_sku = sku_case_by_brand.get(brand_slug, {}).get(sku_key)
...
def rename_files(rename_map: dict[str, str]) -> int:
    ...
    # Stage first to avoid collisions between source and target names.
    for source_name in rename_map:
        source_path = IMAGES_DIR / source_name
        if not source_path.exists():
            continue
        temp_path = source_path.with_suffix(source_path.suffix + ".tmp_catalog_rename")
    ...
    for path in iter_text_files():
        ...
        for match in filename_pattern.finditer(content):
            ...
            if not parsed:
                continue
            brand_slug, extracted_sku, seq = parsed
            canonical_sku = sku_case_by_brand.get(brand_slug, {}).get(extracted_sku.replace(".", "-").lower())
            if not canonical_sku:
                continue
...
def main() -> None:
    ...
    rename_map, conflicts = build_rename_map(sku_case_by_brand)
    renamed = rename_files(rename_map)
    reference_map = build_reference_map_from_text(sku_case_by_brand, rename_map)
    ...
    summary = {
        ...
        "brand_count": len(sku_case_by_brand),
        "sku_count": sum(len(values) for values in sku_case_by_brand.values()),
