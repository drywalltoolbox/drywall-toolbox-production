...
def extract_product_data(product_element):
    ...
    try:
        ...
        return {
            ...
            'image_count': len(local_image_paths),  # Count of actually downloaded images from detail page
...
def scrape_page(page_url, page_num):
    ...
    try:
        ...
        for idx, product_element in enumerate(product_items, 1):
            print(f"\n  Processing product {idx}/{len(product_items)}...")
            product_data = extract_product_data(product_element)
            if product_data:
                page_products.append(product_data)
                print(f"  ✓ [{idx}] {product_data['product_name']} | SKU: {product_data['sku']} | Price: ${product_data['price']} | Images: {product_data['image_count']}")
            else:
        ...
        print(f"\n  Page Summary: {len(page_products)} products successfully extracted")
...
def main():
    print("\n" + "="*100)
    print("PLATINUM DRYWALL TOOLS - PRODUCT SCRAPER V2")
    print("="*100)
    ...
    while current_url and page_num <= 100:  # Safety limit of 100 pages
        ...
        if soup is None:
            print(f"\n⚠ Failed to scrape page {page_num}. Stopping.")
            break
        
        if not page_products:
            print(f"\n⚠ No products found on page {page_num}. Stopping pagination.")
            break
        
        # Add products from this page to the main list
        products.extend(page_products)
        total_products += len(page_products)
        print(f"\n📊 Total products so far: {total_products}")
    ...
    with open(csv_output, 'w', newline='', encoding='utf-8') as f:
        ...
        for product in unique_products:
            writer.writerow([
                ...
                '|'.join(product.get('local_image_paths', [])),
                product['product_url']
            ])
    ...
    if len(unique_products) > 0:
        print(f"Average Images per Product: {sum(p['image_count'] for p in unique_products) / len(unique_products):.2f}")
    ...
    if len(unique_products) > 0:
        ...
        for idx, product in enumerate(unique_products[:5], 1):
            ...
            if product['image_urls']:
                print(f"  Primary URL: {product['image_urls'][0][:70]}...")
            if product.get('local_image_paths'):
                print(f"  Local Images: {len(product['local_image_paths'])} downloaded")
