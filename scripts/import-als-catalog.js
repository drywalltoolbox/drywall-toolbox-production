...
// scripts/import-als-catalog.js
// Read scraped JSONL/JSON and merge into existing CSV product catalog.
// Usage: node import-als-catalog.js --input ./tmp/als-output/als_taping_tools_catalog.jsonl --existing ./public/tswfast_all_products.csv --out ./public/tswfast_all_products_merged.csv --dry-run
...
function mapScrapedToCsvRow(item) {
  // target CSV header used by loadProducts: brand,product_name,part_number,url,image_url,short_description,category,specifications
  ...
  // Remove fields we placed in top-level columns to avoid duplication in specs
...
async function main() {
  ...
  if (fs.existsSync(opts.existing)) {
    ...
    if (parsed.header.length) header = parsed.header; // prefer existing header
  ...
  // flatten map to rows (preserve existing order approximately: existing first, then new)
  const finalRows = [];
  const seen = new Set();
  for (const r of existingRows) {
  ...
  // backup existing if present and not overwriting directly
  if (fs.existsSync(opts.existing)) {
