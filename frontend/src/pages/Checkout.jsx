 ...
 * React owns order intake; WooCommerce/WooPayments owns the final payment UI.
 */
...
const cardVariants = {
  ...
  visible: (delay) => ({
    ...
    y: 0,
    willChange: 'auto',
    transition: { duration: 0.36, ease: [0.16, 1, 0.3, 1], delay: delay ?? 0 },
  }),
};
...
function resolveCartItemImage(item) {
  ...
  return item.image || item.image_src || item.thumbnail || item.image_url || item.product?.image || item.product?.image_src || item.product?.thumbnail || item.images?.[0]?.src || item.images?.[0] || '';
}
        ...
        return (
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            ...
            {idx < CHECKOUT_STEPS.length - 1 && (
              <div className="flex-1 mx-2"><div className="h-px bg-slate-200 relative overflow-hidden rounded-full"><div className="absolute inset-y-0 left-0 bg-primary-600 transition-all duration-500" style={{ width: done ? '100%' : '0%' }} /></div></div>
            )}
          </div>
        );
      })}
...
function SkeletonRow() {
  return <div className="flex justify-between items-start animate-pulse"><div className="flex items-start gap-3 flex-1 mr-4"><div className="h-10 w-10 rounded-xl bg-white/10 shrink-0" /><div className="flex-1 space-y-1.5 pt-0.5"><div className="h-3 bg-white/15 rounded w-3/4" /><div className="h-2.5 bg-white/10 rounded w-1/4" /></div></div><div className="h-3.5 bg-white/15 rounded w-14 shrink-0 mt-0.5" /></div>;
}
                ...
                {cartItems.map((item) => {
                  ...
                  return (
                    <div key={item.cartKey || item.id} className="flex items-center gap-3">
                      <div className="relative h-12 w-12 rounded-xl border border-slate-100 bg-slate-50 overflow-hidden shrink-0">{image ? <img src={image} alt={item.name} className="h-full w-full object-contain p-1" loading="lazy" /> : null}<span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-slate-900 px-1 text-[10px] font-bold text-white">{item.quantity}</span></div>
                      <div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-slate-900">{item.name}</p><p className="text-[11px] text-slate-500 tabular-nums">${toMoneyValue(item.price).toFixed(2)} ea</p></div>
                      <span className="text-xs font-bold text-slate-900 tabular-nums">${(toMoneyValue(item.price) * Number(item.quantity || 1)).toFixed(2)}</span>
                    </div>
                  );
                })}
              </div>
              <div className="px-4 py-3.5 border-t border-slate-100 space-y-1.5 text-xs text-slate-500">
                {[{ label: 'Subtotal', value: `$${subtotal.toFixed(2)}` }, { label: 'Shipping', value: shipping === 0 ? 'Free' : `$${shipping.toFixed(2)}` }, { label: 'Tax (8%)', value: `$${tax.toFixed(2)}` }].map(({ label, value }) => <div key={label} className="flex justify-between"><span>{label}</span><span className={`font-semibold tabular-nums ${label === 'Shipping' && shipping === 0 ? 'text-emerald-600' : 'text-slate-800'}`}>{value}</span></div>)}
    ...
    <aside className="dtb-summary-panel hidden lg:flex flex-col lg:sticky lg:top-6 lg:self-start overflow-hidden rounded-2xl text-slate-100" style={{ background: 'linear-gradient(170deg, #0d1829 0%, #0a1020 80%)' }}>
      ...
      <div className="p-6 border-b border-white/10">
        ...
        <div className="space-y-4 max-h-[360px] overflow-y-auto pr-1">
          {cartItems.length === 0 ? <><SkeletonRow /><SkeletonRow /></> : cartItems.map((item) => {
            const image = resolveCartItemImage(item);
            return <div key={item.cartKey || item.id} className="flex gap-3"><div className="relative h-14 w-14 rounded-xl overflow-hidden bg-white/8 border border-white/10 shrink-0">{image ? <img src={image} alt={item.name} className="h-full w-full object-contain p-1.5" loading="lazy" /> : null}<span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-primary-500 px-1 text-[10px] font-bold text-white">{item.quantity}</span></div><div className="min-w-0 flex-1"><p className="line-clamp-2 text-sm font-semibold text-white leading-snug">{item.name}</p><p className="mt-1 text-[11px] text-slate-400 tabular-nums">${toMoneyValue(item.price).toFixed(2)} ea</p></div><span className="text-sm font-bold text-white tabular-nums shrink-0">${(toMoneyValue(item.price) * Number(item.quantity || 1)).toFixed(2)}</span></div>;
          })}
        </div>
      </div>
      <div className="p-6 space-y-5">
        ...
        <div>
          ...
          <div className="flex gap-2"><input id="desktop-coupon" type="text" value={couponInput} onChange={(event) => setCouponInput(event.target.value.toUpperCase())} placeholder="ENTER CODE" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/10 px-3 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-400/35" /><button type="button" onClick={addManualCoupon} className="rounded-xl bg-white text-slate-950 px-4 py-2.5 text-sm font-bold hover:bg-slate-100 transition-colors">Apply</button></div>
          {manualCoupons.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{manualCoupons.map((code) => <button key={code} type="button" onClick={() => removeManualCoupon(code)} className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-[11px] font-semibold text-emerald-200">{code} ×</button>)}</div>}
        </div>
        <button type="button" onClick={onPlaceOrder} disabled={!canSubmit || processing} className="w-full inline-flex min-h-[52px] items-center justify-center gap-2.5 rounded-xl bg-primary-600 px-5 py-3.5 text-sm font-black text-white shadow-[0_16px_36px_rgba(37,99,235,0.35)] transition-all hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"><Lock size={16} strokeWidth={2.5} />{processing ? 'Preparing Payment…' : 'Continue to Secure Payment'}<ExternalLink size={15} strokeWidth={2.5} /></button>
...
function NativePaymentCard({ gatewayLabel, capabilitiesLoading }) {
  return (
    <StepCard delay={0.21} className="p-5 sm:p-6" id="payment-section">
      <div className="flex items-start gap-3 mb-5"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-primary-50 text-primary-600"><ShieldCheck size={18} strokeWidth={2.2} /></span><div className="min-w-0"><h2 className="text-lg font-black text-slate-950 tracking-tight">Secure Payment</h2><p className="mt-1 text-sm leading-relaxed text-slate-500">Your payment will be completed through the native WooPayments checkout page after order review.</p></div></div>
      <div className="rounded-2xl border border-primary-100 bg-gradient-to-br from-primary-50 via-white to-slate-50 p-4 sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary-500">Payment Gateway</p><p className="mt-1 text-base font-black text-slate-950">{capabilitiesLoading ? 'Loading secure payment…' : gatewayLabel}</p><p className="mt-1 max-w-md text-xs leading-relaxed text-slate-500">Apple Pay, Google Pay, WooPay, saved cards, and other eligible methods are rendered by WooPayments based on device, browser, and store settings.</p></div><div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">{CARD_BRAND_LOGOS.map((logo) => <img key={logo.key} src={logo.src} alt={logo.alt} className={`${logo.className} w-auto object-contain`} loading="lazy" decoding="async" />)}</div></div>
        <div className="mt-4 grid gap-2 sm:grid-cols-3 text-xs text-slate-600"><div className="flex items-center gap-2"><ShieldCheck size={14} className="text-primary-600" />PCI-safe payment</div><div className="flex items-center gap-2"><Lock size={14} className="text-primary-600" />Native gateway page</div><div className="flex items-center gap-2"><PackageCheck size={14} className="text-primary-600" />Order securely held</div></div>
      </div>
    </StepCard>
  );
}
...
export default function Checkout() {
  ...
  const toMoney = useCallback((value) => toMoneyValue(value), []);
  const toNumber = useCallback((value, fallback = 0) => { const n = Number(value); return Number.isFinite(n) ? n : fallback; }, []);
  ...
  useEffect(() => {
    if (!rewardsEnabled || !isAuthenticated || !user?.id) return undefined;
    let mounted = true;
    getUserPoints(user.id).then((data) => {
      if (!mounted) return;
      setPointsBalance(data);
      if (data.points >= POINTS_MIN_REDEEM) setPointsToRedeem(Math.min(Math.floor(data.points / POINTS_REDEEM_STEP) * POINTS_REDEEM_STEP, POINTS_MAX_REDEEM));
    }).catch(() => {});
  ...
  useEffect(() => {
    ...
    getCheckoutCapabilities().then((caps) => {
      if (!mounted) return;
  ...
  useEffect(() => { if (isAddressComplete) fetchShippingRates(formData, safeCartItems); }, [formData.address, formData.city, formData.state, formData.zip, formData.country, fetchShippingRates, isAddressComplete, safeCartItems]);

  const sanitize = (value) => DOMPurify.sanitize(value, { ALLOWED_TAGS: [] });
  const handleInputChange = (event) => { const { name, value } = event.target; setFormData((prev) => ({ ...prev, [name]: sanitize(value) })); if (errors[name]) setErrors((prev) => ({ ...prev, [name]: '' })); };
  const validateForm = useCallback(() => {
    const nextErrors = {};
    if (!formData.firstName.trim()) nextErrors.firstName = 'Required';
    if (!formData.lastName.trim()) nextErrors.lastName = 'Required';
    if (!formData.email.trim()) nextErrors.email = 'Required'; else if (!/\S+@\S+\.\S+/.test(formData.email)) nextErrors.email = 'Invalid email';
  ...
  const roundedMax = Math.min(Math.floor(availablePts / POINTS_REDEEM_STEP) * POINTS_REDEEM_STEP, POINTS_MAX_REDEEM);
  async function handleApplyPoints() {
    ...
    } finally { setApplyingPoints(false); }
  }
  function handleRemovePoints() { setAppliedCoupon(null); setPointsError(''); }
  function addManualCoupon() { const normalized = couponInput.trim().toUpperCase(); if (!normalized) return; setManualCoupons((prev) => (prev.includes(normalized) ? prev : [...prev, normalized])); setCouponInput(''); }
  function removeManualCoupon(code) { setManualCoupons((prev) => prev.filter((current) => current !== code)); }

  const handlePlaceOrder = useCallback(async () => {
    if (!validateForm()) return;
    if (!paymentMethod || isManualPaymentMethod(paymentMethod)) { setCheckoutError('WooPayments is not currently available. Please contact support.'); return; }
    setProcessing(true);
    ...
    try {
      const response = await syncAndPlace(safeCartItems, billingAddress, billingAddress, paymentMethod, [], formData.customerNote, wcRateId, selectedRate ? selectedRate.price : '', [...(appliedCoupon ? [appliedCoupon.code] : []), ...manualCoupons]);
      setStep('placing');
      ...
      if (paymentUrl) { await clearCart(); window.location.assign(paymentUrl); return; }
      setOrderComplete(true);
      await clearCart();
    } catch (error) {
      setCheckoutError(error?.message || 'Checkout failed. Please try again.');
    } finally { setProcessing(false); setStep('form'); }
  }, [appliedCoupon, clearCart, formData, manualCoupons, paymentMethod, safeCartItems, selectedRate, validateForm]);

  useEffect(() => {
    if (processing) { showWorkflow({ label: step === 'syncing' ? 'Preparing secure payment…' : 'Creating your order…', sublabel: 'You will be redirected to the secure WooPayments payment page.', blocking: true }); return; }
    hideWorkflow();
  ...
  if (safeCartItems.length === 0 && !orderComplete) return <Motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="dtb-checkout min-h-screen bg-slate-50 flex items-center justify-center py-16 px-4"><SEOHead noindex title="Checkout" /><div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center max-w-md w-full"><div className="flex h-20 w-20 items-center justify-center rounded-full bg-slate-100 mx-auto mb-6"><ShoppingCart className="h-9 w-9 text-slate-400" strokeWidth={1.5} /></div><h2 className="text-xl font-bold text-slate-900 mb-2">Your cart is empty</h2><p className="text-slate-500 text-sm mb-8">Add products to your cart before checking out.</p><button type="button" onClick={() => navigate('/products')} className="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 active:scale-[0.99] text-white px-7 py-3 rounded-xl font-semibold text-sm transition-all">Browse Products</button></div></Motion.div>;

  if (orderComplete && orderDetails) {
    const wcOrder = orderDetails.wooCommerce;
    const paymentUrl = resolveNativePaymentUrl(wcOrder);
    return <Motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.45 }} className="dtb-checkout min-h-screen bg-slate-50 flex items-center justify-center py-16 px-4"><SEOHead noindex title="Order Created" /><div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 sm:p-10 text-center max-w-md w-full"><div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-emerald-50 mx-auto mb-6"><CheckCircle className="h-10 w-10 text-emerald-500" strokeWidth={1.8} /></div><h2 className="text-2xl font-black text-slate-900 mb-2 tracking-tight">Order Created</h2><p className="text-slate-500 mb-6 text-sm leading-relaxed">Continue to secure payment to complete your order.</p>{paymentUrl ? <a href={paymentUrl} className="inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all">Continue to Secure Payment <ExternalLink size={14} /></a> : <Link to={`/order/${wcOrder?.order_id || wcOrder?.id || ''}`} className="inline-flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all">View Order</Link>}</div></Motion.div>;
  }

  return (
    <div className="dtb-checkout min-h-screen bg-slate-50 page-wrapper"><SEOHead noindex title="Checkout" />
      <div className="mx-auto w-full max-w-[1380px] lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(420px,460px)] lg:gap-6 xl:gap-7 lg:items-start min-h-screen">
        <div className="px-4 py-8 sm:px-7 md:px-8 lg:px-8 xl:px-10 pb-32 lg:pb-16"><div className="max-w-2xl mx-auto lg:mx-0 lg:max-w-[780px]">
          ...
          <div className="space-y-4">
            <StepCard delay={0} className="p-5 sm:p-6"><SectionHeader icon={User} title="Contact Information" /><div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">{[{ name: 'firstName', label: 'First Name', type: 'text', autoComplete: 'given-name' }, { name: 'lastName', label: 'Last Name', type: 'text', autoComplete: 'family-name' }, { name: 'email', label: 'Email Address', type: 'email', autoComplete: 'email' }, { name: 'phone', label: 'Phone', type: 'tel', autoComplete: 'tel', inputMode: 'numeric' }].map(({ name, label, type, autoComplete, inputMode }) => <div key={name}><label htmlFor={`field-${name}`} className={labelClass}>{label}{requiredMark}</label><input id={`field-${name}`} type={type} name={name} value={formData[name]} onChange={handleInputChange} autoComplete={autoComplete} {...(inputMode ? { inputMode } : {})} className={inputClass(name)} aria-invalid={!!errors[name]} />{errors[name] && <p className="text-red-500 text-[11px] mt-1 font-medium" role="alert">{errors[name]}</p>}</div>)}</div></StepCard>
            <StepCard delay={0.05} className="p-5 sm:p-6"><SectionHeader icon={MapPin} title="Shipping Address" /><div className="space-y-3.5"><div><label htmlFor="field-address" className={labelClass}>Street Address{requiredMark}</label><input id="field-address" type="text" name="address" value={formData.address} onChange={handleInputChange} autoComplete="street-address" className={inputClass('address')} aria-invalid={!!errors.address} />{errors.address && <p className="text-red-500 text-[11px] mt-1 font-medium" role="alert">{errors.address}</p>}</div><div className="grid grid-cols-3 gap-3">{[{ name: 'city', label: 'City', autoComplete: 'address-level2' }, { name: 'state', label: 'State', autoComplete: 'address-level1' }, { name: 'zip', label: 'ZIP Code', autoComplete: 'postal-code', inputMode: 'numeric' }].map(({ name, label, autoComplete, inputMode }) => <div key={name}><label htmlFor={`field-${name}`} className={labelClass}>{label}{requiredMark}</label><input id={`field-${name}`} type="text" name={name} value={formData[name]} onChange={handleInputChange} autoComplete={autoComplete} {...(inputMode ? { inputMode } : {})} className={inputClass(name)} aria-invalid={!!errors[name]} />{errors[name] && <p className="text-red-500 text-[11px] mt-1 font-medium" role="alert">{errors[name]}</p>}</div>)}</div>{subtotal > 0 && subtotal < FREE_SHIP_THRESHOLD && <div className="flex items-center gap-2 rounded-xl bg-primary-50 border border-primary-100 px-4 py-2.5 text-xs text-primary-700"><Truck size={13} className="shrink-0" /><span>Spend <strong>${(FREE_SHIP_THRESHOLD - subtotal).toFixed(2)}</strong> more for free shipping</span></div>}</div></StepCard>
            <StepCard delay={0.1} className="p-5 sm:p-6"><SectionHeader icon={Truck} title="Shipping Method" />{ratesLoading && <div className="flex items-center gap-2 text-sm text-slate-400 animate-pulse py-1"><div className="h-2 w-2 rounded-full bg-slate-300 animate-bounce" />Loading shipping options…</div>}{ratesError && !ratesLoading && <div className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-xs text-amber-700"><AlertTriangle size={13} className="shrink-0" />{ratesError}</div>}{!ratesLoading && shippingRates.length === 0 && !ratesError && <p className="text-xs text-slate-400 italic">Enter your address above to see available rates.</p>}{!ratesLoading && shippingRates.length > 0 && <div className="space-y-2" role="radiogroup" aria-label="Shipping method">{shippingRates.map((rate) => <label key={rate.id} className={`flex items-center justify-between gap-3 px-4 py-3.5 rounded-xl border cursor-pointer transition-all ${selectedRate?.id === rate.id ? 'border-primary-500 bg-primary-50/60 ring-1 ring-primary-400/30' : 'border-slate-200 bg-white hover:border-slate-300'}`}><div className="flex items-center gap-3"><input type="radio" name="shippingRate" value={rate.id} checked={selectedRate?.id === rate.id} onChange={() => setSelectedRate(rate)} className="accent-primary-600" /><div><p className="text-sm font-semibold text-slate-900">{rate.name}</p>{rate.eta && <p className="text-xs text-slate-500 mt-0.5">{rate.eta}</p>}</div></div><span className={`text-sm font-bold shrink-0 ${toMoney(rate.price) === 0 ? 'text-emerald-600' : 'text-slate-900'}`}>{toMoney(rate.price) === 0 ? 'Free' : `$${toMoney(rate.price).toFixed(2)}`}</span></label>)}</div>}</StepCard>
            {rewardsEnabled && isAuthenticated && pointsBalance && pointsBalance.points >= POINTS_MIN_REDEEM && <StepCard delay={0.15} className="p-5 sm:p-6"><div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2.5"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-50 text-amber-500"><Star size={14} strokeWidth={2} /></span><h3 className="text-[0.95rem] font-bold text-slate-900">Redeem Points</h3></div><span className="text-xs font-semibold text-slate-500 bg-slate-100 rounded-full px-2.5 py-1">{toNumber(pointsBalance.points, 0)} pts</span></div>{appliedCoupon ? <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3"><div><p className="text-sm font-semibold text-emerald-800">✓ ${toNumber(appliedCoupon.discount_amount, 0).toFixed(2)} discount applied</p><code className="text-xs text-emerald-600 font-mono">{appliedCoupon.code}</code></div><button type="button" onClick={handleRemovePoints} className="text-xs text-red-500 hover:text-red-700 font-semibold ml-3">Remove</button></div> : <><label className="block text-xs text-slate-500 mb-2.5">Redeem <strong className="text-slate-800">{pointsToRedeem} pts</strong> = <strong className="text-emerald-700">${pointsUsd.toFixed(2)} off</strong></label><input type="range" min={POINTS_MIN_REDEEM} max={roundedMax || POINTS_MIN_REDEEM} step={POINTS_REDEEM_STEP} value={pointsToRedeem} onChange={(event) => setPointsToRedeem(Number(event.target.value))} className="w-full mb-3.5 accent-primary-600" />{pointsError && <p className="text-xs text-red-500 mb-2">{pointsError}</p>}<button type="button" onClick={handleApplyPoints} disabled={applyingPoints || pointsToRedeem < POINTS_MIN_REDEEM} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">{applyingPoints ? 'Applying…' : `Apply ${pointsToRedeem} pts`}</button></>}</StepCard>}
            <StepCard delay={0.17} className="p-5 sm:p-6"><SectionHeader icon={Tag} title="Discount Code" /><div className="flex gap-2"><input id="coupon-code" type="text" value={couponInput} onChange={(event) => setCouponInput(event.target.value.toUpperCase())} placeholder="ENTER CODE" className="flex-1 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 transition-all" /><button type="button" onClick={addManualCoupon} className="rounded-xl bg-slate-900 hover:bg-slate-800 px-5 py-3 text-sm font-semibold text-white transition-colors">Apply</button></div>{manualCoupons.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{manualCoupons.map((code) => <button key={code} type="button" onClick={() => removeManualCoupon(code)} className="flex items-center gap-1 rounded-full border border-teal-200 bg-teal-50 px-3 py-1 text-[11px] font-semibold text-teal-700 hover:bg-teal-100 transition-colors"><Tag size={9} /> {code} ×</button>)}</div>}</StepCard>
            <StepCard delay={0.19} className="p-5 sm:p-6"><label htmlFor="field-customerNote" className={labelClass}>Order Note <span className="text-slate-400 normal-case font-normal">(optional)</span></label><textarea id="field-customerNote" name="customerNote" value={formData.customerNote} onChange={handleInputChange} rows={2} placeholder="Special instructions for your order…" className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-sm resize-none placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all hover:border-slate-300" /></StepCard>
            ...
            <div className="lg:hidden fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/96 backdrop-blur px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-14px_34px_rgba(15,23,42,0.10)]"><div className="mx-auto max-w-2xl"><button type="button" onClick={handlePlaceOrder} disabled={!canSubmitCheckout || processing} className="w-full inline-flex min-h-[52px] items-center justify-center gap-2.5 rounded-xl bg-primary-600 px-5 py-3.5 text-sm font-black text-white shadow-[0_16px_36px_rgba(37,99,235,0.28)] transition-all hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"><Lock size={16} strokeWidth={2.5} />{processing ? 'Preparing Payment…' : 'Continue to Secure Payment'}<ExternalLink size={15} strokeWidth={2.5} /></button><p className="mt-2 text-center text-[11px] text-slate-500">Secure payment handled by WooPayments.</p></div></div>
          </div>
        </div></div>
