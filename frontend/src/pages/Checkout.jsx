 ...
 * The React app owns cart review, customer details, shipping, notes, coupons,
 * and order creation. WooCommerce/WooPayments owns the final payment UI.
 */
...
const CARD_BRAND_LOGOS = [
  { key: 'visa', src: VISA_LOGO, alt: 'Visa', className: 'h-[12px]' },
  { key: 'mastercard', src: MASTERCARD_LOGO, alt: 'Mastercard', className: 'h-[13px]' },
  { key: 'amex', src: AMERICAN_EXPRESS_LOGO, alt: 'American Express', className: 'h-[12px]' },
];
                  ...
                  return (
                    <div key={item.cartKey || item.id} className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[11px] text-slate-500 tabular-nums">${toMoneyValue(item.price).toFixed(2)} ea</p>
                      </div>
                      <span className="text-xs font-bold text-slate-900 tabular-nums">
                        ${(toMoneyValue(item.price) * Number(item.quantity || 1)).toFixed(2)}
                      </span>
                    </div>
                  );
                })}
    ...
    >
            ...
            return (
              <div key={item.cartKey || item.id} className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[11px] text-slate-400 tabular-nums">${toMoneyValue(item.price).toFixed(2)} ea</p>
                </div>
      ...
      <div className="p-6 space-y-5">
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 space-y-2.5 text-sm">
          ...
          <div className="flex justify-between text-slate-300">
            <span>Shipping</span>
            <span className={`font-semibold tabular-nums ${shipping === 0 ? 'text-emerald-300' : 'text-white'}`}>{shipping === 0 ? 'Free' : `$${shipping.toFixed(2)}`}</span>
          </div>
          <div className="flex justify-between text-slate-300">
            <span>Tax</span>
            <span className="font-semibold text-white tabular-nums">${tax.toFixed(2)}</span>
          </div>
          <div className="flex justify-between pt-3 border-t border-white/10">
            <span className="font-bold text-white">Total</span>
            <span className="font-black text-2xl text-white tabular-nums">${total.toFixed(2)}</span>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/10 px-3 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-400/35"
            />
            <button
              type="button"
              onClick={addManualCoupon}
              className="rounded-xl bg-white text-slate-950 px-4 py-2.5 text-sm font-bold hover:bg-slate-100 transition-colors"
            >
              Apply
            </button>
          </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {manualCoupons.map((code) => (
                <button
                  ...
                  className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-[11px] font-semibold text-emerald-200"
                >
                  {code} ×
                </button>
              ))}
        ...
        <button
          ...
          className="w-full inline-flex min-h-[52px] items-center justify-center gap-2.5 rounded-xl bg-primary-600 px-5 py-3.5 text-sm font-black text-white shadow-[0_16px_36px_rgba(37,99,235,0.35)] transition-all hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <p className="text-center text-[11px] leading-relaxed text-slate-400">
          Payment opens on the native WooCommerce/WooPayments secure payment page.
        </p>
      </div>
    </aside>
  );
      ...
      <div className="flex items-start gap-3 mb-5">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm leading-relaxed text-slate-500">
            Your payment will be completed through the native WooPayments checkout page after order review.
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-md text-xs leading-relaxed text-slate-500">
              Apple Pay, Google Pay, WooPay, saved cards, and other eligible methods are displayed by WooPayments based on device, browser, and store settings.
            </p>
          </div>
          <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
            {CARD_BRAND_LOGOS.map((logo) => (
              <img key={logo.key} src={logo.src} alt={logo.alt} className={`${logo.className} w-auto object-contain`} loading="lazy" decoding="async" />
            ))}
          </div>
        </div>
      </div>
...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    getCheckoutCapabilities()
      .then((caps) => {
        ...
        const preferredGateway = gateways.find((gateway) => gateway.id === WOO_NATIVE_PAYMENT_METHOD)
          || gateways.find((gateway) => gateway.id === defaultGatewayId)
          || gateways.find((gateway) => !isManualPaymentMethod(gateway))
          || null;
  ...
  ), [isFormComplete, paymentMethod, processing, safeCartItems.length]);
  ...
  }, [formData.address, formData.city, formData.state, formData.zip, formData.country, fetchShippingRates, isAddressComplete, safeCartItems]);
  ...
  useEffect(() => {
    if (processing) {
      showWorkflow({
        label: step === 'syncing' ? 'Preparing secure payment…' : 'Creating your order…',
        sublabel: 'You will be redirected to the secure WooPayments payment page.',
        blocking: true,
      });
      return;
    }
    ...
    return (
      <Motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="dtb-checkout min-h-screen bg-slate-50 flex items-center justify-center py-16 px-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center max-w-md w-full">
          ...
          <button type="button" onClick={() => navigate('/products')} className="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 active:scale-[0.99] text-white px-7 py-3 rounded-xl font-semibold text-sm transition-all">
            Browse Products
          </button>
        </div>
      </Motion.div>
    ...
    return (
      <Motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.45 }} className="dtb-checkout min-h-screen bg-slate-50 flex items-center justify-center py-16 px-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 sm:p-10 text-center max-w-md w-full">
          <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-emerald-50 mx-auto mb-6">
          ...
          {paymentUrl ? (
            <a href={paymentUrl} className="inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all">
              Continue to Secure Payment <ExternalLink size={14} />
            </a>
          ) : (
            <Link to={`/order/${wcOrder?.order_id || wcOrder?.id || ''}`} className="inline-flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all">
              View Order
            </Link>
          )}
        </div>
          ...
          <div className="max-w-2xl mx-auto lg:mx-0 lg:max-w-[780px]">
            <Motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mb-6">
              <div className="flex items-center justify-between mb-1">
                ...
                <Link to="/login" className="text-xs font-semibold text-slate-500 hover:text-slate-800 transition-colors">Sign in</Link>
              </div>
            ...
            <div className="space-y-4">
              ...
              {rewardsEnabled && isAuthenticated && pointsBalance && pointsBalance.points >= POINTS_MIN_REDEEM && (
                <StepCard delay={0.15} className="p-5 sm:p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2.5">
                      <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-50 text-amber-500"><Star size={14} strokeWidth={2} /></span>
                      <h3 className="text-[0.95rem] font-bold text-slate-900">Redeem Points</h3>
                    </div>
                    <span className="text-xs font-semibold text-slate-500 bg-slate-100 rounded-full px-2.5 py-1">{toNumber(pointsBalance.points, 0)} pts</span>
                  </div>
                  {appliedCoupon ? (
                    <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
                      <div>
                        <p className="text-sm font-semibold text-emerald-800">✓ ${toNumber(appliedCoupon.discount_amount, 0).toFixed(2)} discount applied</p>
                        <code className="text-xs text-emerald-600 font-mono">{appliedCoupon.code}</code>
                      </div>
                      <button type="button" onClick={handleRemovePoints} className="text-xs text-red-500 hover:text-red-700 font-semibold ml-3">Remove</button>
                    </div>
                  ) : (
                    <>
                      <label className="block text-xs text-slate-500 mb-2.5">Redeem <strong className="text-slate-800">{pointsToRedeem} pts</strong> = <strong className="text-emerald-700">${pointsUsd.toFixed(2)} off</strong></label>
                      <input type="range" min={POINTS_MIN_REDEEM} max={roundedMax || POINTS_MIN_REDEEM} step={POINTS_REDEEM_STEP} value={pointsToRedeem} onChange={(e) => setPointsToRedeem(Number(e.target.value))} className="w-full mb-3.5 accent-primary-600" />
                      {pointsError && <p className="text-xs text-red-500 mb-2">{pointsError}</p>}
                      <button type="button" onClick={handleApplyPoints} disabled={applyingPoints || pointsToRedeem < POINTS_MIN_REDEEM} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">{applyingPoints ? 'Applying…' : `Apply ${pointsToRedeem} pts`}</button>
                    </>
                  )}
                </StepCard>
              )}
              ...
              <StepCard delay={0.17} className="p-5 sm:p-6">
                ...
                {manualCoupons.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {manualCoupons.map((code) => <button key={code} type="button" onClick={() => removeManualCoupon(code)} className="flex items-center gap-1 rounded-full border border-teal-200 bg-teal-50 px-3 py-1 text-[11px] font-semibold text-teal-700 hover:bg-teal-100 transition-colors"><Tag size={9} /> {code} ×</button>)}
                  </div>
                )}
              </StepCard>
              ...
              {checkoutError && (
                <Motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 flex items-start gap-2">
